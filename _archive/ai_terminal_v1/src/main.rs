...
use terminal_ui::{App, TerminalSession};
...
async fn main() -> Result<()> {
    ...
    let mut terminal_session = TerminalSession::new(ollama_client, python_bridge)?;
    info!("Terminal session created successfully");
    terminal_session.set_system_tools_executor(tools_tx_arc);
    info!("System tools executor configured");
    
    if let Some(model_name) = model {
        info!("Setting model to: {}", model_name);
        terminal_session.set_model(model_name.clone()).await?;
    }
    
    terminal_session.set_offline_mode(offline_mode);
    info!("About to start terminal application");
    
    // Create and run the main application
    let mut app = App::new(terminal_session);
    app.run().await?;
